...
namespace yltl.DBUtility
{
    ...
    /// Sybase 数据库操作类
    /// </summary>
    public class SybaseHelper : yltl.DBUtility.IDataBaseOper
    {
        /// <summary>
        /// 默认是 oracle 的数据库连接
        /// </summary>
        private static SybaseHelper dbOper_Sybase; //access 数据库的连接
        private static SybaseHelper dbOper_Custom;//自定义的数据库连接


        protected OleDbTransaction _Trans;//事务处理
        ...
        public SybaseHelper(string ConnectionString)
        {
        ...
        public SybaseHelper(eDBType at2, string ServerName, string DataBaseName, string UserIdName, string Password)
        {
            switch (at2)
            {
                ...
                case eDBType.Sybase:
                    ...
                    this.strConntionString = temp;// "Provider=Sybase.ASEOLEDBProvider.2;Data Source=" + ServerName + ";Initial Catalog=" + DataBaseName + ";User ID=" + UserIdName + ";Password=" + Password + "";
                    break;
                default:
                    break;
            }
        ...
        private void connOpen()
        {
            try
            {
                ...
                {
                    oleconn = new OleDbConnection(strConntionString);
                    oleconn.Open();
                }
                else
                {
        ...
        /// </summary>
        /// <param name="sql">需要执行的sql语句</param>
        /// <param name="dr">out 传入的 OleDbDataReader 无需初始化该对象 </param>
        ...
        public DbDataAdapter GetDataAdapter(string sql)
        {
            ...
            {
                connOpen();
                da = new OleDbDataAdapter(sql, oleconn);
            }
            catch (OleDbException oledbex)
            {
                throw new Exception(oledbex.Message);
        ...
        public DataTable GetDataTable(string sql)
        {
            ...
            try
            {
                ...
                connOpen();
                //logger.Error("11111." + sql);
                da = new OleDbDataAdapter(sql, oleconn);
                //logger.Error("22222." + sql);
        ...
        public bool ExecuteSqlTran(ArrayList sqlList)
        {
            ...
            OleDbTransaction tx = oleconn.BeginTransaction();
            cmd.Transaction = tx;
            try
            {
        ...
        public void ExecuteProc(string procName, DbParameter[] prams, bool blTrans)
        {
            ...
            try
            {
                ...
                cmd = new OleDbCommand(procName, oleconn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Transaction = _Trans;
                foreach (OleDbParameter param in prams)
        ...
        /// <summary>
        /// 执行存储过程，返回OleDbDataReader对象
        /// </summary>
        /// <param name="procName">存储过程名称</param>
        /// <param name="prams">参数集</param>
        /// <param name="blTrans">是否启动事务处理</param>
        /// <returns>返回OleDbDataReader对象</returns>
        ...
        {
            using (OleDbConnection connection = new OleDbConnection(strConntionString))
            {
                using (OleDbCommand cmd = new OleDbCommand(sqlText, connection))
                {
                    try
                    {
                        connection.Open();
        ...
        /// 执行多条SQL语句，实现数据库事务。
        /// </summary>
        /// <param name="sqlTextList">SQL语句的哈希表（key为sql语句，value是该语句的OleDbParameter[]）</param>
        public void ExecuteSqlTran(Hashtable sqlTextList)
        {
            using (OleDbConnection conn = new OleDbConnection(strConntionString))
            {
                conn.Open();
        ...
        {
            OleDbConnection connection = new OleDbConnection(strConntionString);
            OleDbCommand cmd = new OleDbCommand();
            try
            {
                PrepareCommand(cmd, connection, null, sqlText, cmdParms);
        ...
        private static void PrepareCommand(OleDbCommand cmd, OleDbConnection conn, OleDbTransaction trans, string cmdText, DbParameter[] cmdParms)
        {
            ...
            string queryString, string whereString, string orderString, out int pageCount, ref int recordCount)
        {
            using (OleDbConnection connection = new OleDbConnection(strConntionString))
            {
                ...
                OleDbCommand cmdCount = new OleDbCommand(countSQL, connection);
